...
@Component({
  selector: 'app-view-transcript-data',
  templateUrl: './view-transcript-data.component.html',
  styleUrls: ['./view-transcript-data.component.css']
})
export class ViewTranscriptDataComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  ngOnInit() {
   ...
   // to apply color for spotter keyword W.R.T to categoty
    //  this.transcripService.questionnaireList.forEach(category => {
    //    category.keywords.forEach(keyword => {
    //      var keysArray = [];
    //      keysArray = keyword.split(" ");
    //      this.transcript.forEach((word, index) => {
    //      if (keysArray.length === 1) {
    //        if (keysArray[0] === word.Word) {
    //          word['Color'] = category.keyWordsColor;
    //        }
    //      } else {
    //        for (var key = 0; key < keysArray.length; key++) {
    //          if (keysArray[key] === word.Word) {
    ...
    //  })
    //});
    for (var item = 0; item < this.transcripService.questionnaireList.length; item++) {
      if (this.transcripService.questionnaireList[item].keywords.length > 0) {
        for (var key = 0; key < this.transcripService.questionnaireList[item].keywords.length; key++) {
          var keysArray = [];
          this.transcripService.questionnaireList[item].keywords[key] = this.transcripService.questionnaireList[item].keywords[key].trim();
          keysArray = this.transcripService.questionnaireList[item].keywords[key].split(" ");
          if (keysArray.length === 1) {
            if (this.transcript[0].Speaker === "Speaker 0") {
              for (var s = 0; s < this.transcript.length; s++) {
                if (keysArray[0] === this.transcript[s].Word) {
                  this.transcript[s]['Color'] = this.transcripService.questionnaireList[item].keyWordsColor;
                  //console.log(this.transcript[s]);
                }
              }
            }
          } else {
            if (this.transcript.length > 0 && this.transcript[0].Speaker === "Speaker 0") {
              if (this.transcript.length >= keysArray.length) {
              for (var word = 0; word < this.transcript.length; word++) {
                this.transcript[word].Word = (this.transcript[word].Word).toLowerCase();
                if (keysArray.length > 0) {
                  keysArray[0] = (keysArray[0]).toLowerCase();

                  if (keysArray[0] === this.transcript[word].Word) {
                    if (this.transcript.length > keysArray.length) {
                      var next = word + 1;
                      if (keysArray.length <= (this.transcript.length - word)) {
                           if (keysArray[1] === this.transcript[next].Word) {
                          for (var i = (keysArray.length - 1); i >= 0; i--) {
                            var tar = word + i;
                            if ((keysArray[i]).toLowerCase() === (this.transcript[tar].Word).toLowerCase()) {
                              this.transcript[word + i]['Color'] = this.transcripService.questionnaireList[item].keyWordsColor;
                            //  console.log(this.transcript[word + i]);
                            }
                          }
                        }
                      }
    ...
    if (this.transcript[0].Speaker === 'Speaker 1' ) {
      this.transcript['StartTime'] = this.transcript[0].Begin;
      this.transcript['EndTime'] = this.transcript[this.transcript.length - 1].Begin + this.transcript[this.transcript.length - 1].Duration;
    } else {
      if (this._dataService.countStart) {
        this.transcript['StartTime'] = 0;
        this.transcript['EndTime'] = this._dataService.customerEndPoints[this._dataService.count];
        this._dataService.count++;
      }
    }
    this.consolidatedTranscript.push(this.transcript);
